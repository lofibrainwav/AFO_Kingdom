...
class SystemMonitor:
        ...
        if "check" in config:
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=5.0) as client:
                    ...
                    result["status"] = (
                        "UP" if response.status_code < 400 else "DEGRADED"
                    )
    ...
    async def monitor_once(self) -> dict:
        ...
        # 서비스 상태 확인
        service_tasks = [
            self.check_service(name, config) for name, config in self.services.items()
        ]
        ...
        while True:
            try:
                ...
                for service_name, service_data in result["services"].items():
                    ...
                    print(
                        f"  {status_icon} {service_name}: {service_data['status']}{time_info}"
                    )
