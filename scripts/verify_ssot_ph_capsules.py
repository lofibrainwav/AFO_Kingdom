...
def main() -> None:
    if len(sys.argv) != 2:
        die(
            "usage: python3 scripts/verify_ssot_ph_capsules.py docs/AFO_EVOLUTION_LOG.md"
        )
    ...
    hdr_re = re.compile(
        r"^## \[SSOT/PH-[A-Z0-9-]+/\d{4}-\d{2}-\d{2}/([0-9a-f]{7,40})?\] .+$"
    )
    ...
    hdr_idx = [i for i, l in enumerate(t) if l.startswith("## [SSOT/PH-")]
    if not hdr_idx:
        die(
            "❌ SSOT/PH capsules not found (0). This must FAIL to prevent silent regression."
        )
    ...
    for i in hdr_idx:
        ...
        m = re.match(
            r"^## \[SSOT/PH-[A-Z0-9-]+/\d{4}-\d{2}-\d{2}/([0-9a-f]{7,40})?\] ", header
        )
