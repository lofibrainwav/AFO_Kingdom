...
class HealthDashboard:
    ...
    def create_dashboard(self, data: Dict[str, Any]) -> Panel:
        ...
        table.add_row("시스템 상태", data.get('overall_status', 'unknown'), "✅" if data.get('overall_status') == 'healthy' else "⚠️")
    ...
    async def run_dashboard(self):
        ...
        try:
            ...
            if self.metrics_history:
                ...
                healthy_checks = sum(1 for h in self.metrics_history 
                                   if h['data'].get('overall_status') == 'healthy')
                avg_performance = sum(h['data'].get('ollama_health', {}).get('performance_ms', 0) 
                                    for h in self.metrics_history) / total_checks
                
                console.print(f"총 체크 수: {total_checks}")
                console.print(f"정상 상태: {healthy_checks}/{total_checks} ({healthy_checks/total_checks*100:.1f}%)")
