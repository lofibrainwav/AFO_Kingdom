...
class HealthDashboard:
    ...
    def create_dashboard(self, data: Dict[str, Any]) -> Panel:
        ...
        table.add_row(
            "Trinity Score", f"{total_score:.1f}%", "✅" if total_score > 15 else "⚠️"
        )
        table.add_row(
            "Ollama 연결성", connectivity, "✅" if connectivity == "✅" else "❌"
        )
        table.add_row(
            "응답 시간", f"{performance:.1f}ms", "✅" if performance < 100 else "⚠️"
        )
        table.add_row(
            "시스템 상태",
