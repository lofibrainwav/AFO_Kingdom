...
SEARCH_PATTERNS = [
    ...
    "Trinity CI",
    "GenUI",
    "TICKET-045", "TICKET-046", "TICKET-047",
]
...
def http_get_json(path: str):
    ...
    try:
        with urlopen(req, timeout=3) as r:
            ...
            try:
                return {"ok": True, "url": url, "status": r.status, "json": json.loads(raw)}
...
def http_get_json(path: str):
    ur         idx = data.find(pat)
            if idx != -1:
                start = max(0, idx - 80)
                end = min(len(data), idx + 200)
                snippet = data[start:end].replace("\n", "\\n")
                hits.append({"file": str(p.relative_to(ROOT)), "patter            except json.JSONDecodeError:
                return {"ok": True, "url": url, "sT%H:%M:%S%z")
    payload = {
        "as_of": ts,
        "base_url": BASE_URL,
    ...
    print(json.dumps({"wrote": str(out_path), "as_of": ts, "hits": len(payload["repo_scan_hits"])}, ensure_ascii=False, indent=2))
