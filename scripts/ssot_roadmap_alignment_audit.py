...
def http_probe(path: str, timeout=5):
    ...
    try:
        with urlopen(req, timeout=timeout) as r:
            ...
            snap = tximport json,         return {"ok": True, "url": url, "status": getattr(r, "status", None), "ms": ms, "snapshot": snap}
        ...
        try:
            ...
            body = ""
        return {"ok": False, "url": url, "status": getattr(e, "code", None), "ms": ms, "error": str(e), "snapshot": body}
    except (URLError, TimeoutError) as e:
        ms = (perf_counter() - t0) * 1000.0
        return {"ok": False, "url": url, "status": None, "ms": ms, "error": str(e)}

def find_trinity_refs(limit=20):
    hits = []
...
def main():
    ...
    payload = {
        ...
        "roadmap": {
            ...
            "sha256": sha256_file(roadmap_path)             if p.suffix) else None,
        },
        "endpoints": {
