...
def run_simulation(query):
    print(f"🧠 [Sim] Loading Kingdom Knowledge Base (AFO_EVOLUTION_LOG.md)...")
    time.sleep(1)
    ...
    if "accuracy" in query.lower() or "bert" in query.lower():
        ...
        answer = "Phase 10은 Matrix Stream Visualization으로, 실시간 사고 시각화(SSE)를 구현했습니다."
        source = "walkthrough.md"
    else:
        answer = "왕국의 기록에 따르면, 현재 시스템은 Trinity Score 85% 이상을 유지하며 순항 중입니다."
        source = "General Logs"
