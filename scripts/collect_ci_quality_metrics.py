...
def main() -> int:
    ...
    lines.append(f'afo_report_english_ratio{{repo="{REPO}"}} {english_ratio} {now}')

    lines.append("# TYPE afo_pr_risk_score gauge")
    lines.append(f'afo_pr_risk_score{{repo="{REPO}"}} {pr_risk_score} {now}')

    lines.append("# TYPE afo_chaos_nightly_success gauge")
    lines.append(f'afo_chaos_nightly_success{{repo="{REPO}",test_type="lite"}} {chaos_success} {now}')

    # Histogram SSOT 유지 + A1용 gauge 파생치
    lines.append("# TYPE afo_chaos_selfheal_seconds_last gauge")
    lines.append(f'afo_chaos_selfheal_seconds_last{{repo="{REPO}",test_type="lite"}} {chaos_self_heal_seconds_last} {now}')

    # Counter 대신 임시 gauge (A1)
    lines.append("# TYPE afo_report_ssot_gate_fail_24h gauge")
    lines.append(f'afo_report_ssot_gate_fail_24h{{repo="{REPO}"}} {ssot_fail_24h} {now}')
