...
class ObsidianTemplateValidator:
    ...
    def __init__(self, vault_path: str = "docs"):
    ...
    def validate_mermaid_syntax(self, content: str) -> list[str]:
        ...
        for i, block in enumerate(mermaid_blocks):
            ...
            valid_patterns = [
                r'\bgraph\b', r'\bflowchart\b',  # 그래프 다이어그램
                r'\bsequenceDiagram\b',  # 시퀀스 다이어그램
                r'\bstateDiagram\b', r'\bstateDiagram-v2\b',  # 상태 다이어그램
                r'\bpie\b',  # 파이 차트
                r'\bgantt\b'  # 간트 차트
            ]
    ...
    def generate_summary(self, results: dict) -> dict:
        ...
        # 시스템 상태 결정
        template_success_rate = summary['valid_templates'] / summary['total_templates'] if summary['total_templates'] > 0 else 0
...
def main():
    ...
    try:
        ...
        print(f"  ⚙️ 옵시디언 설정 파일: {sum(structure['obsidian_config_files'].values())}/{len(structure['obsidian_config_files'])}")

        # 종합 결과 출력
        summary = results['summary']
        print("\n🎯 종합 결과:")
        print(f"  📊 템플릿 검증율: {summary['valid_templates']}/{summary['total_templates']} ({summary['valid_templates']/summary['total_templates']*100:.1f}%)")
