    ...
    for f, items in by_file.items():
        ...
        for ln in targets:
            ...
            for back in range(1, 9):  # 8줄 위까지
                ...
                if cand.strip() == "":
                    continue
                if cand.lstrip().startswith(("return ", "yield ", "if ", "elif ", "else:", "for ", "while ", "with ", "try:", "except", "finally:")):
                    continue
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python3 scripts/pyright_patch_unknown_variable_v732_upscan.py <pyright_report.txt>")
        raise SystemExit(2)
