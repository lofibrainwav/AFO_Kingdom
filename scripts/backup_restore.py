...
class BackupManager:
    ...
    def create_backup(
        self, name: str = None, include_non_critical: bool = False
    ) -> str:
        """백업 생성"""
        if name is None:
    ...
    def restore_backup(self, backup_name: str, target_dir: str = None) -> bool:
        ...
        if target_dir is None:
            target_dir = (
                f"restore_{backup_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
...
def main():
    ...
    parser.add_argument(
        "--keep-days", type=int, default=30, help="보관할 백업 일수 (cleanup용)"
    )
        ...
        if args.action == "backup":
            ...
            if backups:
                ...
                for backup in backups:
                    size_mb = backup["size"]
                    print(
                        f"  📁 {backup['name']} ({size_mb:.1f}MB) - {backup['created'][:19]}"
                    )
            else:
                print("📦 사용 가능한 백업 없음")
