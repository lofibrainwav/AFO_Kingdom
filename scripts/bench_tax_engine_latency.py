...
def build_kwargs(fn, payload):
  ...
  for pname, p in params.items():
    if p.kind in (p.VAR_POSITIONAL, p.VAR_KEYWORD):
      continue
    if pname in payload:
      kwargs[pname] = payload[pname]  "agelif p.default is inspect._emp  "age": 30
}

ng_required.append(pname)
  if missing_required:
    return None, missing_required
  return ("kw    r, kwargs), None
...
def run_once(payload):
  ...
  if fn is None:
    raise RuntimeError("CALLABLE_FAIL: No callable method found on TaxCalculator (tried calculate/compute/calc/run/__call__)")
...
def main():
  ...
  out_path = Path(args.out) if args.out else Path("artifacts") / f"bench_tax_engine_latency_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl"
  ...
  with out_path.open("w", encoding="utf-8") as f:
    for i in range(args.runs):
      ...
      samples_ms  if args.payload:
    payload.u  "ticket": "PERF_BENCH_TAX_ENGINE",
        "run": i + 1,
        "latency_ms": ms,
  ...
  summary_path = out_path.with_suffix(".summary.json")
      "measured_runs": args.runsumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
