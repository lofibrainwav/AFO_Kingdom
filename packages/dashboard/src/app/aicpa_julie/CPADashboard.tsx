          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-100 tracking-tight">
                AICPA Julie's Office
              </h1>
              <div className="flex items-center gap-2">
                <p className="text-xs text-gray-500 uppercase tracking-widest">
                  Royal Finance Department
                </p>
                {loading && (
                  <span className="text-xs text-amber-500 animate-pulse">• Syncing...</span>
                )}
              </div>
              <p className="text-xs text-emerald-400 mt-1 italic opacity-90">
                "{displayData.advice}"
              </p>
            </div>
          </div>
        ...
        {displayData.risk_alerts && displayData.risk_alerts.length > 0 && (
          <div className="mb-6 grid gap-2">
            {displayData.risk_alerts.map((alert: any, i: number) => (
              <div
                key={i}
                className={`p-3 rounded-lg border ${alert.level === "warning" ? "bg-red-500/5 border-red-500/20 text-red-300" : "bg-blue-500/5 border-blue-500/20 text-blue-300"} flex items-center gap-3 shadow-sm`}
              >
            ...
            <div className="bg-[#0A0F1C] border border-gray-800 rounded-2xl p-6 space-y-4 shadow-lg">
              <div>
                ...
                <div className="text-xl font-mono text-white tracking-wider">
                  ₩ {displayData.monthly_spending?.toLocaleString()}
                </div>
              </div>
              <div>
                ...
                <div className="text-xl font-mono text-emerald-400 tracking-wider">
                  ₩ {displayData.budget_remaining?.toLocaleString()}
                </div>
              </div>
            </div>
          </div>
