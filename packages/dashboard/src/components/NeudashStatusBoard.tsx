  ...
  if (error)
    return (
      <div className="p-8 text-center text-red-500">
        Failed to load Kingdom Status. Backend might be offline.
      </div>
    );
  if (isLoading || !data)
    return <div className="p-8 text-center text-neu-text">Connecting to AFO Core...</div>;
    ...
    <div className="flex min-h-screen bg-platinum text-neu-text font-sans selection:bg-neu-primary selection:text-white">
      ...
      <aside className="hidden lg:flex w-64 flex-col p-8 gap-8 fixed h-full overflow-y-auto">
        ...
        <nav className="flex flex-col gap-4">
          ...
          <div className="neu-btn justify-start bg-transparent shadow-none hover:shadow-[6px_6px_12px_var(--shadow-dark),-6px_-6px_12px_var(--shadow-light)]">
            Chancellor Rules
          </div>
          <div className="neu-btn justify-start bg-transparent shadow-none hover:shadow-[6px_6px_12px_var(--shadow-dark),-6px_-6px_12px_var(--shadow-light)]">
            Trinity Metrics
      ...
      <main className="flex-1 p-4 lg:p-8 lg:ml-64">
        ...
        <header className="flex justify-between items-center mb-12">
          <div>
            <div className="text-neu-light text-sm font-medium mb-1">
              Kingdom / Overview / Deep Research
            </div>
            <h1 className="text-3xl font-bold tracking-tight text-neu-text">Status Report 2025</h1>
          </div>
          <div className="neu-icon-box w-12 h-12 text-lg">👑</div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          ...
          <div className="neu-card flex flex-col items-center text-center">
            <div className="text-neu-light font-semibold text-sm mb-2 uppercase tracking-wide">
              System Heartbeat
            </div>
          ...
          <div className="neu-card flex flex-col items-center text-center">
            ...
            <div className="text-5xl font-extrabold text-neu-text mb-4">
              {dependency_count}
              <span className="text-2xl text-neu-light">/{total_dependencies}</span>
            </div>
            <div className="neu-pill text-neu-primary">
              <span className="w-2 h-2 rounded-full bg-neu-primary shadow-[0_0_8px_rgba(90,103,216,0.8)]"></span>{" "}
              VERIFIED
            </div>
          </div>
                  ...
                  <div className="neu-icon-box w-20 h-20 text-4xl mb-4">
                    {scholar.name === "Jaryong"
                      ...
                      : scholar.name === "Bangtong"
                        ? "🐥"
                        : scholar.name === "Yeongdeok"
                          ? "🛡️"
                          : "🏛️"}
                  </div>
                  <div className="font-bold text-neu-text">{scholar.name}</div>
                  <div className="text-xs text-neu-light mt-1 uppercase font-semibold">
                    {scholar.role}
                  </div>
                </div>
              ))}
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
            {verified_dependencies.map((dep) => (
              <div
                key={dep}
                className="group p-4 rounded-2xl bg-platinum shadow-[6px_6px_12px_var(--shadow-dark),-6px_-6px_12px_var(--shadow-light)] hover:-translate-y-1 transition-transform border border-white/40 flex items-center gap-3 text-sm font-semibold text-neu-text"
              >
