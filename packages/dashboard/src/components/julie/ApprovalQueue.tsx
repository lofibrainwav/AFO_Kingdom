    ...
    return (
        <div className="bg-[#0A0F1C] border border-gray-800 rounded-2xl overflow-hidden flex flex-col">
             <div className="p-6 border-b border-gray-800 flex justify-between items-center bg-gradient-to-r from-amber-900/10 to-transparent">
                ...
                <span className="text-xs font-bold text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded-full">{items.length} Pending</span>
            </div>

            <div className="p-4 space-y-3">
                {items.length === 0 ? (
                    <div className="text-center py-8 text-gray-600 text-sm">No pending approvals. The desk is clear.</div>
                ) : (
                    items.map(item => (
                        <div key={item.id} className="bg-gray-900/50 border border-gray-800 rounded-xl p-4">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className="font-mono text-amber-400 font-bold">{item.amount.toLocaleString()} ₩</span>
                            </div>
                            <p className="text-xs text-gray-500 mb-3">{item.reason} • <span className="text-gray-400">Req: {item.requester}</span></p>
