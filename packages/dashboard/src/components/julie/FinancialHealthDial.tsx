...
export default function FinancialHealthDial({ score, trend, risk_level }: FinancialHealthDialProps) {
    ...
    return (
        <div className="bg-[#0A0F1C] border border-gray-800 rounded-2xl p-6 relative overflow-hidden backdrop-blur-xl">
            {/* Background Glow */}
            <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-[80px] opacity-20 ${score >= 90 ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>

            <div className="flex justify-between items-start mb-4">
                <div>
                    <h3 className="text-gray-400 text-sm font-medium tracking-wider uppercase">Financial Trinity Score</h3>
            ...
            <div className="flex items-baseline gap-2 mb-2">
                <span className={`text-5xl font-black ${score >= 90 ? 'text-emerald-400' : 'text-amber-400'}`}>
                    {score.toFixed(1)}
                </span>
            ...
            <div className="flex items-center gap-2 mt-2">
                ...
                <div className="text-gray-600 text-xs ml-auto">
                    Risk: <span className={`font-bold ${getRiskColor(risk_level)} uppercase`}>{risk_level}</span>
                </div>
            </div>
