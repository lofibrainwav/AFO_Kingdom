...
export default function ChancellorView() {
  ...
  const refreshHealth = useCallback(async () => {
    ...
    try {
      const [healthData, familyRes] = await Promise.all([fetchHealthStatus(), fetchFamilyStatus()]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logError("Chancellor invocation failed", {
        error: err instanceof Error ? err.message : "Unknown error",
      });
      setResponse({
        speaker: "System",
        response:
          "Error communicating with the Chancellor. Ensure backend Docker services are running.",
        full_history: [],
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 space-y-6">
      ...
      <div
        className={`bg-black/90 border-2 ${activePersona.border} rounded-lg p-4 ${getDynamicGlow()} transition-all duration-1000`}
      >
        <div className="flex items-center justify-between mb-3">
          <h2
            className={`${activePersona.color} font-mono text-xl tracking-wider flex items-center gap-2`}
          >
            <span className="animate-pulse">●</span> {activePersona.icon}
          </h2>
          {health && (
            <span
              className={`font-mono text-sm ${DECISION_COLORS[health.decision] || "text-gray-400"}`}
            >
              {health.decision}
            </span>
          )}
        ...
        <div className="grid grid-cols-5 gap-2 text-xs font-mono mb-3">
          ...
          <div className="border border-green-800/50 p-2 text-center">
            <div className="text-green-400 text-lg">
              {formatPercent(health?.trinity?.filial_serenity)}
            </div>
          ...
          <div className="border border-purple-800/50 p-2 text-center">
            <div className="text-purple-400 text-lg">
              {formatPercent(health?.trinity?.eternity)}
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 text-xs font-mono text-gray-400">
          <div className="border border-gray-800 p-2">
            TRINITY_SCORE:{" "}
            <span className="text-green-400">{health?.health_percentage?.toFixed(1) ?? "-"}%</span>
          </div>
          <div className="border border-gray-800 p-2">
            STATUS:{" "}
            <span className={health?.status === "balanced" ? "text-green-400" : "text-yellow-400"}>
        ...
        <div className="bg-black/80 border border-gray-800 rounded-lg p-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {Object.entries(familyData.members).map(([key, member]) => (
              <div
                key={key}
                className={`border border-gray-800 p-3 rounded bg-gray-900/50 flex align-top gap-3 ${member.status === "WORKING" ? "border-green-900/50 shadow-[0_0_10px_rgba(34,197,94,0.1)]" : ""}`}
              >
                <div className="text-2xl font-none">{member.name === "Julie" ? "👩‍💼" : "🤖"}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-gray-300 font-mono text-xs font-bold truncate pr-2">
                      {member.name}
                    </span>
                    <span
                      className={`text-[10px] font-mono border px-1.5 py-0.5 rounded-full whitespace-nowrap ${member.status === "WORKING" ? "border-green-600 text-green-400 bg-green-950/30" : "border-gray-700 text-gray-500"}`}
                    >
                      {member.status}
                    </span>
                  </div>
                  <p
                    className={`text-xs font-mono truncate ${STATUS_COLORS[member.status] || "text-gray-500"}`}
                  >
                  ...
                  <div className="mt-2 flex gap-2 text-[9px] text-gray-500 font-mono bg-black/50 p-1 rounded w-fit">
                    <span className="text-cyan-400">
                      眞{Math.round((member.pillars?.truth || 0) * 100)}
                    </span>
                    <span className="text-amber-400">
                      善{Math.round((member.pillars?.goodness || 0) * 100)}
                    </span>
                    <span className="text-pink-400">
                      美{Math.round((member.pillars?.beauty || 0) * 100)}
                    </span>
                  </div>
                </div>
              </div>
      ...
      {/* Main Display Area */}
      <div
        className={`bg-black border-2 ${activePersona.border} rounded-lg min-h-[400px] p-6 relative overflow-hidden transition-colors duration-500`}
      >
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <div
              className={`flex items-center gap-2 ${activePersona.color} font-mono text-sm border-b border-white/10 pb-2`}
            >
              <span
                className={`${activePersona.bg} px-2 py-0.5 rounded text-xs border ${activePersona.border} bg-opacity-20`}
              >
