  ...
  if (loading || !data) return (
      <div className="glass-card p-8 animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
             {[1,2,3,4,5].map(i => <div key={i} className="h-32 bg-white/5 rounded-xl"></div>)}
          </div>
      </div>
  );
  ...
  return (
    <div className="glass-card p-8 mt-8 border-t border-white/10">
      <h3 className="text-2xl font-bold text-center mb-8 flex items-center justify-center gap-2">
         ...
         <span className="px-2 py-0.5 rounded text-xs bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">Live Metrics</span>
      </h3>
      
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {data.pillars_detailed.map((p) => (
          <div key={p.key} className="group relative p-5 bg-gradient-to-b from-white/5 to-transparent rounded-2xl border border-white/5 hover:border-cyan-500/30 transition-all hover:bg-white/10">
            {/* Header */}
