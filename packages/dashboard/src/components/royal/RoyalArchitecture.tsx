        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="p-3 border border-dashed border-slate-400 rounded-xl bg-slate-50 w-full max-w-2xl text-center">
            ...
            <div className="flex justify-center gap-4 flex-wrap">
              <span className="px-3 py-1 bg-white rounded shadow-sm text-xs font-mono">
                Bangtong (Codex)
              </span>
              <span className="px-3 py-1 bg-white rounded shadow-sm text-xs font-mono">
                Jaryong (Claude)
...
const FlowStep = ({ title, desc, highlight }: any) => (
  <div className="relative">
    <div
      className={`absolute -left-[29px] top-1 w-3 h-3 rounded-full border-2 border-white ${highlight ? "bg-indigo-500 ring-2 ring-indigo-200" : "bg-slate-300"}`}
    />
    <div className={`text-sm font-bold ${highlight ? "text-indigo-600" : "text-slate-600"}`}>
      {title}
    </div>
    <div className="text-xs text-slate-400">{desc}</div>
  </div>
