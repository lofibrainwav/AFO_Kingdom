...
export default function SkillDeck() {
  ...
  if (!data && !error)
    return <div className="p-4 text-xs text-slate-400">Loading Royal Skills...</div>;
    ...
    <div className="flex overflow-x-auto pb-8 pt-4 gap-6 scrollbar-hide snap-x px-4 h-full items-center">
      {skills.map((skill, index) => (
        ...
        >
          ...
          <div
            className={clsx(
              "absolute top-0 left-0 right-0 h-16 opacity-20 transition-opacity group-hover:opacity-40",
          ...
          <div className="z-10 mt-2">
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/40 border border-white/60 text-slate-500 uppercase tracking-wider">
                {skill.category}
              </span>
            </div>
          </div>
