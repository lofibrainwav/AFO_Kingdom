...
class VisionResult:
    def __init__(self, passed: bool, screenshot_path: str, errors: list):
        self.passed = passed
...
class VisionVerifier:
    ...
    async def verify_url(self, url: str, name: str) -> VisionResult:
        ...
        async with async_playwright() as p:
            ...
            errors = []
            page.on("console", lambda msg: errors.append(f"Console {msg.type}: {msg.text}") if msg.type == "error" and "hydration" not in msg.text.lower() else None)
            page.on("pageerror", lambda exc: errors.append(f"Page Error: {exc}") if "hydration" not in str(exc).lower() else None)
