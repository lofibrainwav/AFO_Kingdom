...
class JuyuAgent:
    def __init__(self):
        ...
        self.tools = {
            "refactor_widget": self._tool_refactor_widget
        }
    ...
    async def _tool_refactor_widget(self, relative_path: str) -> str:
        ...
        if "packages/dashboard" not in relative_path:
             full_path = os.path.join(project_root, "packages/dashboard/src/components", relative_path)
        else:
    ...
    def _calculate_beauty_score(self, code: str) -> int:
        score = 70 # Base
        if "glass-card" in code: score += 10
        if "backdrop-blur" in code: score += 5
        if "gradient-to" in code: score += 5
        if "lucide-react" in code: score += 5
        if "animate-" in code: score += 5
