...
# SSOT BASE_CONFIG (merge_node.py에서 추출)
BASE_CONFIG = {
    "weights": {
        "truth": 0.35,  # 眞 (Truth)
        "goodness": 0.35,  # 善 (Goodness)
        "beauty": 0.20,  # 美 (Beauty)
        "serenity": 0.08,  # 孝 (Serenity)
        "eternity": 0.02,  # 永 (Eternity)
    },
    "thresholds": {
        "auto_run_trinity": 90,
        "auto_run_risk": 10,
...
def apply_learning_profile(
    ...
    if "thresholds" in profile_overrides:
        ...
        # 기본 검증: trinity >= 80, risk <= 20
        base_validation = (
            isinstance(thresholds_override, dict)
            and 80 <= thresholds_override.get("auto_run_trinity", 90) <= 100
            and 0 <= thresholds_override.get("auto_run_risk", 10) <= 20
        )
