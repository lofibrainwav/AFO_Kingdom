...
def _calculate_federal_standard_deduction_2025(
    ...
    status = tax_input.filing_status.upper()

    # 기본 표준공제
    base_deduction = float(base_deductions[status])
    ...
    # 고령자/시각장애인 추가 공제
    additional_config = federal_params["additional_std_deduction_aged_blind"]
    ...
    if status in ("SINGLE", "HEAD_OF_HOUSEHOLD"):
        per_box_amount = float(additional_config["single_or_hoh"])
        base_deduction += per_box_amount * boxes
    else:  # MARRIED_FILING_JOINTLY, MARRIED_FILING_SEPARATELY, QUALIFYING_SURVIVING_SPOUSE
        per_box_amount = float(additional_config["married_or_qss"])
...
def _calculate_ca_standard_deduction_2025(params: dict[str, Any], tax_input: TaxInput) -> float:
    ...
    return float(deductions[status])
...
class TaxCalculator:
    ...
    def calculate_federal_tax_2025(self, tax_input: TaxInput) -> tuple[float, float, float]:
        ...
        # 세금 계산
        filing_status = tax_input.filing_status.upper()
        brackets = self.params["federal"]["brackets"][filing_status]
        tax = _progressive_tax(taxable_income, brackets)

        return (tax, taxable_income, deduction)

    def calculate_ca_tax_2025(self, tax_input: TaxInput) -> tuple[float, float, float]:
        """캘리포니아 주세 계산 (세금, 과세소득, 공제액 반환)"""
        ...
        # CA 세금 계산
        filing_status = tax_input.filing_status.upper()
        brackets = self.params["ca"]["brackets"][filing_status]
        tax = _progressive_tax(taxable_income, brackets)
