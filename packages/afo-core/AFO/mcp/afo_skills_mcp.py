...
@mcp.tool()
def skills_list(category: str | None = None, q: str | None = None, limit: int = 100) -> dict:
...
@mcp.tool()
def skills_detail(skill_id: str) -> dict:
    if not skill_id:
...
@mcp.tool()
def skills_execute(skill_id: str, inputs: dict | None = None, dry_run: bool = True) -> dict:
    if not skill_id:
        raise ValueError("skill_id is required")
...
@mcp.tool()
def genui_generate(prompt: str, template: str = "react_component", dry_run: bool = True) -> dict:
