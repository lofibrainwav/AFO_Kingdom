...
def check_irmaa_risk(magi: int, filing_status: FilingStatus) -> tuple[bool, Optional[int]]:
    """
    IRMAA 리스크 체크
    
    Medicare Part B 프리미엄 과부담 여부 평가
    """
...
def calculate_tax(input_data: TaxInput) -> TaxResult:
    ...
    眞 (Truth): 정확한 IRS 세율 적용
    善 (Goodness): 최적화 제안 포함
    """
    # 1. 과세 소득 계산
...
def generate_advice(
    ...
    if marginal_rate <= 0.12 and headroom > 0:
        advice_parts.append(
            f"✨ OBBBA Sweet Spot 활용 가능! "
            f"12% 구간까지 ${headroom:,} 여유가 있습니다."
        )
