...
class MatchingEngine:
    ...
    def __init__(self, skeleton_index: SkeletonIndex):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python matching_engine.py <md_file>")
        return
    ...
    try:
        ...
        if result.best_match:
            print(f"Best match: {result.best_match.module.path}")
            print(f"Similarity: {result.best_match.similarity_score:.2f}")
            print(f"Reason: {result.best_match.match_reason}")
