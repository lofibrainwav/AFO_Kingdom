...
class RoyalLibrarySkill:
        ...
        **kwargs: Any
        ...
        if simulate:
            ...
            return PrincipleResult(
                ...
                classic=Classic.SUN_TZU,
                success=True,
                message=f"[DRY_RUN] 시뮬레이션 완료 - 실제 실행 전 검토 필요",
                data={
    ...
    async def principle_05_trinity_alignment(
        ...
        return PrincipleResult(
            ...
            classic=Classic.SUN_TZU,
            success=aligned,
            message=f"Trinity Score: {trinity_score:.2%}" + (" - 정렬됨" if aligned else " - 정렬 필요"),
            data={"trinity_score": trinity_score, "眞": truth_score, "善": goodness_score, "美": beauty_score},
            trinity_impact={"眞": 0.70, "孝": 0.30}
        )

    async def principle_06_standard_then_custom(self) -> PrincipleResult:
        """[06] 정병 - 정석으로 공격하고, 변칙으로 승리하라."""
        return PrincipleResult(6, "정병", Classic.SUN_TZU, True, "표준 패턴 준수 후 오버라이딩", {}, {"眞": 0.70, "孝": 0.30})

    async def principle_07_find_bottleneck(self, metrics: dict[str, float] | None = None) -> PrincipleResult:
        """[07] 허실 - 시스템의 병목을 찾아 집중 보강하라."""
        metrics = metrics or {}
        bottleneck = min(metrics.items(), key=lambda x: x[1])[0] if metrics else "unknown"
        return PrincipleResult(7, "허실", Classic.SUN_TZU, True, f"병목 발견: {bottleneck}", {"metrics": metrics}, {"眞": 0.70, "孝": 0.30})

    async def principle_08_exception_handler(self, error: Exception) -> PrincipleResult:
        """[08] 구변 - 예외 상황에 따라 유연하게 대처 경로를 바꿔라."""
        return PrincipleResult(8, "구변", Classic.SUN_TZU, True, f"예외 처리: {type(error).__name__}", {"error": str(error)}, {"眞": 0.70, "孝": 0.30})
    ...
    async def principle_09_logging_spy(self, message: str, level: str = "info") -> PrincipleResult:
        ...
        return PrincipleResult(9, "용간", Classic.SUN_TZU, True, f"[{level.upper()}] 로그 기록 완료", {"message": message}, {"眞": 0.70, "孝": 0.30})

    async def principle_10_dangerous_action_gate(self, action_name: str, confirmed: bool = False) -> PrincipleResult:
        """[10] 화공 - 파괴적 행동은 확실한 이득이 있을 때만 수행하라."""
        return PrincipleResult(10, "화공", Classic.SUN_TZU, confirmed, f"위험 행동: {action_name}" + (" - 승인됨" if confirmed else " - 승인 필요"), {"confirmed": confirmed}, {"眞": 0.70, "孝": 0.30})

    async def principle_11_mvp_deploy(self) -> PrincipleResult:
        """[11] 졸속 - 완벽하게 늦는 것보다, 부족해도 빨리 배포하고 고치는 게 낫다."""
        return PrincipleResult(11, "졸속", Classic.SUN_TZU, True, "MVP 배포 우선", {}, {"眞": 0.70, "孝": 0.30})

    async def principle_12_full_automation(self) -> PrincipleResult:
        """[12] 부전이굴 - 최고의 자동화는 사용자가 아무것도 하지 않게 하는 것이다."""
        return PrincipleResult(12, "부전이굴", Classic.SUN_TZU, True, "완전 자동화 목표", {}, {"眞": 0.70, "孝": 0.30})
    ...
    async def principle_13_loose_coupling(self, modules: list[str]) -> PrincipleResult:
        ...
        return PrincipleResult(13, "도원결의", Classic.THREE_KINGDOMS, True, f"{len(modules)}개 모듈 느슨한 결합", {"modules": modules}, {"永": 0.60, "善": 0.40})
    ...
    async def principle_15_graceful_degradation(self, fallback_value: Any) -> PrincipleResult:
        ...
        return PrincipleResult(15, "공성계", Classic.THREE_KINGDOMS, True, "Graceful Degradation 활성화", {"fallback": str(fallback_value)[:100]}, {"永": 0.60, "善": 0.40})

    async def principle_16_external_integration(self, package: str) -> PrincipleResult:
        """[16] 초선차전 - 남의 자원(오픈소스, 외부 API)을 빌려 내 힘으로 삼아라."""
        return PrincipleResult(16, "초선차전", Classic.THREE_KINGDOMS, True, f"외부 리소스 활용: {package}", {"package": package}, {"永": 0.60, "善": 0.40})

    async def principle_17_pipeline_chain(self, steps: list[str]) -> PrincipleResult:
        """[17] 연환계 - 작은 마이크로 서비스들을 연결하여 거대한 함대를 만들어라."""
        return PrincipleResult(17, "연환계", Classic.THREE_KINGDOMS, True, f"{len(steps)}단계 파이프라인", {"steps": steps}, {"永": 0.60, "善": 0.40})

    async def principle_18_hide_complexity(self) -> PrincipleResult:
        """[18] 미인계 - 복잡한 백엔드 로직은 아름다운 UI 뒤에 숨겨라."""
        return PrincipleResult(18, "미인계", Classic.THREE_KINGDOMS, True, "복잡성 추상화 완료", {}, {"永": 0.60, "善": 0.40})

    async def principle_19_feedback_loop(self, iteration: int = 1) -> PrincipleResult:
        """[19] 칠종칠금 - 사용자가 만족할 때까지 끈질기게 수정하고 피드백을 받아라."""
        return PrincipleResult(19, "칠종칠금", Classic.THREE_KINGDOMS, True, f"피드백 루프 #{iteration}", {"iteration": iteration}, {"永": 0.60, "善": 0.40})

    async def principle_20_scheduled_task(self, cron: str = "0 * * * *") -> PrincipleResult:
        """[20] 동남풍 - 타이밍이 생명이다. 스케줄러를 활용하라."""
        return PrincipleResult(20, "동남풍", Classic.THREE_KINGDOMS, True, f"스케줄 설정: {cron}", {"cron": cron}, {"永": 0.60, "善": 0.40})

    async def principle_21_circuit_breaker(self, threshold: int = 5) -> PrincipleResult:
        """[21] 고육지계 - 시스템 전체를 살리기 위해 일부 기능을 희생할 줄 알아야 한다."""
        return PrincipleResult(21, "고육지계", Classic.THREE_KINGDOMS, True, f"Circuit Breaker 임계값: {threshold}", {"threshold": threshold}, {"永": 0.60, "善": 0.40})

    async def principle_22_code_convention(self, linter: str = "ruff") -> PrincipleResult:
        """[22] 한실부흥 - 코드의 정통성(Legacy)과 스타일 가이드를 준수하라."""
        return PrincipleResult(22, "한실부흥", Classic.THREE_KINGDOMS, True, f"린터: {linter}", {"linter": linter}, {"永": 0.60, "善": 0.40})

    async def principle_23_modular_split(self, parts: int = 3) -> PrincipleResult:
        """[23] 천하삼분 - 거대한 문제는 쪼개어 정복하라."""
        return PrincipleResult(23, "천하삼분", Classic.THREE_KINGDOMS, True, f"{parts}개로 분할", {"parts": parts}, {"永": 0.60, "善": 0.40})

    async def principle_24_checkpoint_state(self, state: dict[str, Any]) -> PrincipleResult:
        """[24] 탁고 - 자신이 종료되더라도, 다음 프로세스를 위해 상태를 남겨라."""
        return PrincipleResult(24, "탁고", Classic.THREE_KINGDOMS, True, "상태 저장 완료", {"state_keys": list(state.keys())}, {"永": 0.60, "善": 0.40})
    ...
    async def principle_26_error_control(self) -> PrincipleResult:
        ...
        return PrincipleResult(26, "비르투", Classic.THE_PRINCE, True, "Exception Handling 강화", {}, {"善": 0.50, "眞": 0.50})

    async def principle_27_algorithm_select(self, options: list[str]) -> PrincipleResult:
        """[27] 여우와 사자 - 때로는 교활하게, 때로는 강력하게 해결하라."""
        return PrincipleResult(27, "여우와사자", Classic.THE_PRINCE, True, f"알고리즘 옵션: {options}", {"options": options}, {"善": 0.50, "眞": 0.50})
    ...
    async def principle_28_ux_friction_check(self, friction_score: float) -> PrincipleResult:
        ...
        return PrincipleResult(28, "증오피하기", Classic.THE_PRINCE, safe, f"Friction: {friction_score}" + (" ✅" if safe else " ⚠️"), {"friction": friction_score}, {"善": 0.50, "眞": 0.50})

    async def principle_29_executable_code_only(self) -> PrincipleResult:
        """[29] 무장한 예언자 - 코드 없는 아이디어는 패배한다. 실행 가능한 코드만 가치가 있다."""
        return PrincipleResult(29, "무장한예언자", Classic.THE_PRINCE, True, "Executable Code Only", {}, {"善": 0.50, "眞": 0.50})

    async def principle_30_garbage_collect(self) -> PrincipleResult:
        """[30] 잔인함의 효율적 사용 - 좀비 프로세스나 낭비 자원은 가차 없이 죽여라."""
        return PrincipleResult(30, "잔인함", Classic.THE_PRINCE, True, "Garbage Collection 완료", {}, {"善": 0.50, "眞": 0.50})

    async def principle_31_uptime_monitor(self, uptime_percent: float = 99.9) -> PrincipleResult:
        """[31] 국가 유지 - 시스템의 Uptime 유지가 군주의 제1덕목이다."""
        return PrincipleResult(31, "국가유지", Classic.THE_PRINCE, uptime_percent >= 99.0, f"Uptime: {uptime_percent}%", {"uptime": uptime_percent}, {"善": 0.50, "眞": 0.50})

    async def principle_32_model_router(self, models: list[str]) -> PrincipleResult:
        """[32] 현명한 조언자 - 좋은 모델을 선택하고, 나쁜 모델은 걸러라."""
        return PrincipleResult(32, "현명한조언자", Classic.THE_PRINCE, True, f"모델 풀: {models}", {"models": models}, {"善": 0.50, "眞": 0.50})
    ...
    async def principle_33_creative_solution(self, trinity_score: float) -> PrincipleResult:
        ...
        return PrincipleResult(33, "결과정당화", Classic.THE_PRINCE, allowed, f"Trinity: {trinity_score:.0%}" + (" - 파격 허용" if allowed else " - 정석 유지"), {"trinity_score": trinity_score}, {"善": 0.50, "眞": 0.50})
    ...
    async def principle_35_complexity_estimate(self, task: str, factors: int = 3) -> PrincipleResult:
        """[35] 마찰 - 이론상 쉬워 보여도 실제로는 어렵다. 마찰계수를 계산하라."""
        return PrincipleResult(35, "마찰", Classic.ON_WAR, True, f"마찰 요소: {factors}개 (작업: {task[:30]})", {"task": task, "factors": factors}, {"眞": 0.60, "孝": 0.40})
    ...
    async def principle_37_resource_monitor(self, usage_percent: float) -> PrincipleResult:
        ...
        return PrincipleResult(37, "공세종말점", Classic.ON_WAR, safe, f"리소스: {usage_percent:.1f}%" + (" ✅" if safe else " ⚠️ 재정비 필요"), {"usage": usage_percent}, {"眞": 0.60, "孝": 0.40})

    async def principle_38_singleton_lock(self, resource: str) -> PrincipleResult:
        """[38] 지휘 통일 - 명령 권한은 하나여야 한다. 중복 실행을 막아라."""
        return PrincipleResult(38, "지휘통일", Classic.ON_WAR, True, f"싱글톤 락: {resource}", {"resource": resource}, {"眞": 0.60, "孝": 0.40})
    ...
    async def principle_39_token_optimize(self, tokens: int, limit: int = 4096) -> PrincipleResult:
        ...
        return PrincipleResult(39, "병력절약", Classic.ON_WAR, efficient, f"토큰: {tokens}/{limit}" + (" ✅" if efficient else " ⚠️"), {"tokens": tokens, "limit": limit}, {"眞": 0.60, "孝": 0.40})

    async def principle_40_auto_run_gate(self, confidence: float, threshold: float = 0.8) -> PrincipleResult:
        """[40] 대담함 - 확률이 높다면, 과감하게 자동화를 질러라."""
        auto_run = confidence >= threshold
        return PrincipleResult(40, "대담함", Classic.ON_WAR, auto_run, f"신뢰도: {confidence:.0%}" + (" → AUTO_RUN" if auto_run else " → 수동 확인"), {"confidence": confidence, "threshold": threshold}, {"眞": 0.60, "孝": 0.40})
    ...
    async def principle_41_business_alignment(self, business_value: str) -> PrincipleResult:
        ...
        return PrincipleResult(41, "정치연속", Classic.ON_WAR, True, f"비즈니스 가치: {business_value}", {"value": business_value}, {"眞": 0.60, "孝": 0.40})
