...
class UnifiedSovereigntyGate:
    ...
    def __init__(
        self, min_trinity: float = 90.0, max_risk: float = 10.0, max_gap: float = 0.30
    ):
    ...
    def check_sovereignty(
        self, trinity: float, risk: float, gap: float
    ) -> dict[str, Any]:
        """
    ...
    def __repr__(self):
        return f"UnifiedSovereigntyGate(T>={self.min_trinity}, R<={self.max_risk}, G<{self.max_gap})"
