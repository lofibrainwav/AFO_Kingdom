...
class Persona(BaseModel):
    ...
    trinity_score: PersonaTrinityScore = Field(
        default_factory=PersonaTrinityScore,
        description="페르소나별 Trinity Score"
    )
    
    # 맥락 정보
    context: PersonaContext = Field(
        default_factory=PersonaContext,
    ...
    trinity_os_persona_id: str | None = Field(
        default=None,
        description="TRINITY-OS 페르소나 ID (연동용)"
    )
...
class PersonaLogEntry(BaseModel):
    ...
    trinity_score: PersonaTrinityScore | None = Field(
        default=None,
        description="Trinity Score (있는 경우)"
    )
