...
router = APIRouter(prefix="/aicpa", tags=["AICPA Agent Army"])
...
class MissionRequest(BaseModel):
    ...
    command: Optional[str] = Field(None, description="추가 명령 (자연어)")
...
async def execute_aicpa_mission(request: MissionRequest):
    ...
    try:
        ...
        logger.error(f"[AICPA] Mission failed: {str(e)}")
...
async def simulate_tax(request: TaxSimulationRequest):
    ...
    try:
        ...
        logger.error(f"[AICPA] Tax simulation failed: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def simulate_roth_ladder_strategy(request: RothLadderRequest):
    ...
    try:
        ...
        logger.error(f"[AICPA] Roth Ladder simulation failed: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_client_report(request: ReportRequest):
    ...
    try:
        ...
        logger.error(f"[AICPA] Report generation failed: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_client_data(client_name: str):
    ...
    try:
        ...
        logger.error(f"[AICPA] Client lookup failed: {str(e)}")
