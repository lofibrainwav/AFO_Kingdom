...
async def get_current_pillars():
    ...
    try:
        ...
        # Overall 계산 (제3계명 공식)
        overall = (
            scores_dict["truth"] * 0.3
            + scores_dict["goodness"] * 0.3
            + scores_dict["beauty"] * 0.2
            + scores_dict["serenity"] * 0.15
            + scores_dict["forever"] * 0.05
        )
...
print("✅ 5기둥 API Router loaded successfully")
print("   - GET /api/5pillars/current (현재 5기둥 점수 조회)")
print("   - POST /api/5pillars/live (LangFlow 실시간 평가 - Phase 23-D)")
print("   - GET /api/5pillars/family/hub (가족 허브 전체 상태 - Phase 23-E)")
print("   - POST /api/5pillars/family/hub/member/update (가족 구성원 데이터 업데이트)")
print("   - GET /api/5pillars/family/hub/data (실시간 가족 허브 데이터)")
