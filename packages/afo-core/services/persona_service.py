...
class PersonaService:
    ...
    @validate_with_trinity
    async def get_persona_from_db(self, persona_id: str) -> dict[str, Any] | None:
        """
    ...
    async def _send_via_mcp(self, log_entry: dict[str, Any]) -> None:
        """
    ...
    async def _send_via_sse(self, log_entry: dict[str, Any]) -> None:
        """
...
async def get_current_persona() -> dict[str, Any]:
    """현재 페르소나 조회 편의 함수"""
    result: dict[str, Any] = await persona_service.get_current_persona()
    return result
