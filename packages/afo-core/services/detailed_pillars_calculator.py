...
class DetailedPillarsCalculator:
    ...
    def calculate_beauty_score(self, code_snippet: str, context: Dict[str, Any]) -> float:
        ...
        if "_" in code_snippet and not "Variable" in code_snippet: # Very rough heuristic
             score += 0.2
        elif context.get("style_guide_passed"):
             score += 0.2
             
        # 3. UX Elegance (Max 0.3) - For UI code or System Status
        if context.get("ux_theme") == "Glassmorphism" or "Trinity Glow" in context.get("features", []):
             score += 0.3
