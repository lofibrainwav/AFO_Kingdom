...
class TrinitySimulator:
    ...
    def simulate_real_time(self, changes: dict[str, int] | None = None) -> dict[str, Any]:
        ...
        if "error_occurrence" in changes:
            ...
            logger.debug(f"[TrinitySimulator] 에러 발생: 기술적 완성도 {changes['error_occurrence']}점")
        ...
        if "config_update" in changes:
            ...
            logger.debug(f"[TrinitySimulator] 설정 업데이트: 실현 가능성 +{changes['config_update']}점")
        ...
        if "philosophy_add" in changes:
            ...
            logger.debug(f"[TrinitySimulator] 철학 기능 추가: 철학 구현 +{changes['philosophy_add']}점")
        ...
        if "consistency_improve" in changes:
            ...
            logger.debug(f"[TrinitySimulator] 시스템 정합성 개선: +{changes['consistency_improve']}점")
