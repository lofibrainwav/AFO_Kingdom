...
print("DEBUG: Executing AFO/api_wallet.py top level code")
...
class APIWallet:
    ...
    def add(
        ...
        Raises:
            ValueError: If key name already exists
        """
        # Encrypt the API key
        ...
        if self.use_db:
            ...
            # Add ID
            key_data["id"] = len(storage["keys"]) + 1
            key_id = int(str(key_data["id"]))
    ...
    def _release_connection(self, conn: object) -> None:
