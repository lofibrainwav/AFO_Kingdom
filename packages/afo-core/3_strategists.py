...
class QueryModel(BaseModel):
    ...
    context: Dict[str, Any] = Field(default_factory=dict, description="컨텍스트 데이터 - 아키텍처 검증용")
...
class ThreeStrategists:
    ...
    def zhuge_liang_truth_evaluate(query_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            arch_fit = 1.0 if "valid_structure" in model.context else 0.8  # 아키텍처 타당성 상세 평가
            type_coverage = min(1.0, model.validation_level / 10)  # 검증 강도에 따라 점수 조정 (MyPy 연동 시뮬)
            return (arch_fit + type_coverage) / 2  # Dry_Run 성공: 0.9~1.0

        except ValidationError as e:
            log_sse(f"[제갈량 眞] 상세 검증 실패: {e} - 타당성 위반, 재검토 필요")  # SSE 상세 로그 (미 달성)
    ...
    def sima_yi_goodness_review(query_data: Dict[str, Any]) -> float:
        ...
        if risk_level > 0.1:
            log_sse(f"[사마의 善] 상세 리스크 위반: {risk_level} - 윤리/보안 게이트 차단, ASK_COMMANDER 제안")  # SSE 상세 로그
            return 0.0
    ...
    async def parallel_strategist_thinking(query_data: Dict[str, Any]) -> float:
        ...
        log_sse(f"[3책사 병렬 사고 상세 완료] Raw: {raw_scores} → Trinity: {final_score} (지속 시간: {duration:.2f}s)")
        return final_score
