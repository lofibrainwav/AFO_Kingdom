...
from playwright.async_api import async_playwright, Browser, Playwright, Route, Request, TimeoutError as PlaywrightTimeout
    ...
    async def verify_ui(
        ...
        try:
            ...
            if enable_tracing:
                trace_path = screenshot_path.replace(".png", "_error_trace.zip")
                await context.tracing.stop(path=trace_path)
            raise HTTPException(status_code=500, detail=f"UI 검증 실패: {str(e)}")
        finally:
            # 모킹 정리 (일회성 시나리오라면)
            if mock_scenarios:
