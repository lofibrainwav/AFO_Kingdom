...
class TestHealthEndpoint:
    ...
    def test_health_endpoint_has_trinity(self, client):
        """GET /health 응답에 trinity 필드가 있는지 테스트"""
        response = client.get("/health")
        data = response.json()
        assert "trinity" in data

    def test_trinity_score_range(self, client):
    ...
    def test_health_percentage_range(self, client):
    ...
    def test_organs_present(self, client):
        """organs 필드가 있는지 테스트"""
        response = client.get("/health")
        data = response.json()
        assert "organs" in data

    def test_decision_field(self, client):
