...
def test_get_api_usage_success():
...
def test_get_api_usage_import_error():
    ...
    with patch.dict(sys.modules):
        ...
        # The code does `from afo_soul_engine import api_server`
...
def test_billing_summary_success():
    with patch.dict(sys.modules, {"afo_soul_engine.api_server": MagicMock()}):
...
def test_save_browser_token_success():
...
def test_get_extraction_script_missing():
...
def test_extract_session_invalid_provider():
    response = client.post(
