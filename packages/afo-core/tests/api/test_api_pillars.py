...
class TestPillarsEndpoint:
    ...
    def test_pillars_current_returns_200(self, client):
        """GET /api/5pillars/current 가 200을 반환하는지 테스트"""
        response = client.get("/api/5pillars/current")
        assert response.status_code == 200

    def test_pillars_current_has_required_fields(self, client):
    ...
    def test_pillars_live_post(self, client):
    ...
    def test_family_hub_returns_200(self, client):
        """GET /api/5pillars/family/hub 테스트"""
        response = client.get("/api/5pillars/family/hub")
        assert response.status_code == 200

    def test_family_hub_has_data(self, client):
...
class TestTrinityScoreCalculation:
    ...
    def test_trinity_formula_weights(self):
    ...
    def test_trinity_partial_scores(self):
        """부분 점수 계산 테스트"""
        # 모든 기둥이 0.5일 때
