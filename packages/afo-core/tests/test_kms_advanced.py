...
def test_init_success():
    ...
    with patch("AFO.kms.vault_kms.hvac", mock_hvac_module):
        with patch.dict(os.environ, {"VAULT_ADDR": "http://test", "VAULT_TOKEN": "token"}):
            kms = VaultKMS()
...
def test_get_encryption_key_unavailable():
    kms = VaultKMS()
    # No client
    assert kms.get_encryption_key() is None


def test_get_encryption_key_error():
    kms = VaultKMS()
    kms.client = MagicMock()
    kms.client.is_authenticated.return_value = True
...
def test_set_encryption_key_success():
    kms = VaultKMS()
    kms.client = MagicMock()
    kms.client.is_authenticated.return_value = True
...
def test_set_encryption_key_error():
    kms = VaultKMS()
    kms.client = MagicMock()
    kms.client.is_authenticated.return_value = True
