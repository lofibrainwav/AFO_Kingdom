...
async def test_task_group_exception_propagation():
    """Verify that multiple exceptions are captured in an ExceptionGroup."""
    ...
    assert "Task A failed" in str(excinfo.value.exceptions)
    assert "Task B failed" in str(excinfo.value.exceptions)
