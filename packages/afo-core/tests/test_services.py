...
class TestHybridRAGService:
    ...
    def test_rag_query_structure(self):
        """RAG 쿼리 구조 테스트"""
        query = {"question": "테스트 질문", "top_k": 5, "include_sources": True}
        assert "question" in query
        assert query["top_k"] > 0

    def test_rag_response_structure(self):
        """RAG 응답 구조 테스트"""
        response = {"answer": "테스트 답변", "sources": [], "confidence": 0.85}
