...
def test_kill_switch_always_wins_over_forced_on():
...
def test_header_forced_off_beats_flag_and_gradual():
...
def test_header_forced_on_beats_everything_except_kill():
    headers = {"X-AFO-RAG": "1", "X-AFO-CLIENT-ID": "u1"}
    env = {
        "AFO_RAG_KILL_SWITCH": "0",
...
def test_gradual_respects_percent_and_is_stable_for_same_seed():
...
def test_gradual_disabled_means_shadow_only():
    headers = {"X-AFO-CLIENT-ID": "u1"}
    env = {
        "AFO_RAG_KILL_SWITCH": "0",
        "AFO_RAG_FLAG_ENABLED": "0",
