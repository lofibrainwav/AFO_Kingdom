...
class JulieConfig(BaseSettings):
    ...
    MAX_FRICTION_THRESHOLD: float = Field(
        default=30.0, description="Max friction before blocking"
    )
    ...
    MIN_TX_ID_LENGTH: int = Field(
        default=10, description="Minimum length for Transaction ID"
    )
    MIN_DESC_LENGTH: int = Field(
        default=5, description="Minimum length for Description"
    )
