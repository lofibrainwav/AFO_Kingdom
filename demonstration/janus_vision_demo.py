...
async def run_demo():
    ...
    try:
        ...
        print(
            f"  - Action Sequence: {[r['action']['type'] for r in result['results']]}"
        )
        print(f"  - Final Summary: {result.get('summary', '조사가 완료되었습니다.')}")
        print("-" * 40 + "\n")

        print(
            "✅ [Janus] 시각적 인지 루프가 이제 '그림자'를 벗어나 '실재'하는 힘임을 증명했습니다."
        )
...
if __name__ == "__main__":
    if os.getenv("JANUS_VISION_ENABLED") != "1":
        print(
            "⚠️ JANUS_VISION_ENABLED is not set in environment. Setting it for demo..."
        )
