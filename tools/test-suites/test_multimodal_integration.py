...
def test_multimodal_integration():
    ...
    print(f"   Vision Service: {'Available' if engine.vision_service else 'Not Available'}")
    print(f"   Audio Service: {'Available' if engine.audio_service else 'Not Available'}")
    ...
    result = engine.add_image(image_path, description="Test image for multimodal RAG", analyze=True)
    if result:
        print(f"   ✅ Image added successfully with vision analysis")
    else:
    ...
    result = engine.add_audio(audio_path, description="Test audio for multimodal RAG", transcribe=True)
    if result:
        print(f"   ✅ Audio added successfully with transcription")
    else:
    ...
    for i, doc in enumerate(results, 1):
        print(f"      {i}. Type: {doc.content_type}, Path: {doc.metadata.get('path', 'N/A')[:50]}")
    ...
    print(f"   Memory Usage: {stats['memory_stats']['current_memory_mb']} MB / {stats['memory_stats']['max_memory_mb']} MB")
    print(f"   Memory Utilization: {stats['memory_stats']['memory_utilization'] * 100:.1f}%")
